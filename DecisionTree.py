...
def getPertinentExamples(examples,attrName,attrValue):
    ...
    """
    newExamples=filter(lambda x:x[attrName]==attrValue,examples)
    #YOUR CODE HERE
    return newExamples
...
def getClassCounts(examples,className):
    ...
    classCounts = {}
    for e in examples:
        if not e[className] in classCounts:
            classCounts[e[className]]=0
        val=classCounts[e[className]]
        classCounts[e[className]]=val+1
    return classCounts
...
def setEntropy(classCounts):
    ...
    for c in classCounts:
        print (c/float(s))
        print c
        entropy=entropy+(c/float(s))*log((c/float(s)),2)
    return -entropy
def remainder(examples,attrName,attrValues,className):
    ...
    #YOUR CODE HERE
    for a in attrValues: 
        c=getAttributeCounts(examples,attrName,attrValues,className)
        (c/sum)*log((c/sum),2)
